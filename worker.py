...
class Worker:
    ...
    def init_kvcache_comm(self,mscclpp_init_method: Optional[str] = None) -> None:
        if mscclpp_init_method is not None:
            # Issac: Set up here
            from vllm.worker.new_comm_utils import NewKVCacheCommManager
            self.kvcache_comm_manager = NewKVCacheCommManager(
                self.rank, self.parallel_config.world_size,
                self.parallel_config.num_prompt_workers, self.device)
    ...
    def setup_kvcache_comm(self) -> None:
        ...
        if self.kvcache_comm_manager is not None:
            ...
            # Issac added here
            self.kvcache_comm_manager.setup_comm(num_layers, self.gpu_cache, self.cache_engine.block_size)

            # Populate the attention modules with the KV cache communicator.
            self.set_comm_for_attention_modules()
            # Issac added here
            self.kvcache_comm_manager.warm_up(self.is_token_worker())
    ...
    def execute_model(
        ...
        if len(blocks_to_nw) and self.is_token_worker() and not is_prompt:
            for sem_id in blocks_to_nw:
                #TODO: add loop for both headers
